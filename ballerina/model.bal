...
public type ChatAssistantMessage record {|
    ...
    # Required unless `toolCalls` is specified
    string? content = ();
    # An optional name for the participant
    # Provides the model information to differentiate between participants of the same role
    string name?;
...
public isolated client class OpenAiModel {
    *Model;
    ...
    isolated remote function chat(ChatMessage[] messages, ChatCompletionFunctions[] tools, string? stop = ())
        ...
        if tools.length() > 0 {
            request.tools = self.mapChatCompletionFunctionsToChatCompletionTools(tools);
        }
        ...
        chat:ChatCompletionMessageToolCalls? openAiToolCalls = message?.tool_calls;
        ToolCall[]? toolCalls = openAiToolCalls is chat:ChatCompletionMessageToolCalls
            ? self.mapOpenAiToolCallsToToolCalls(openAiToolCalls) : ();
        return {role: ASSISTANT, toolCalls, content: message?.content};
    }
        ...
        foreach ChatMessage message in messages {
            if message is ChatAssistantMessage {
                ...
                if toolCalls is ToolCall[] {
                    chat:ChatCompletionMessageToolCall[] chatCompletionToolCall = toolCalls.'map(toolCall => <chat:ChatCompletionMessageToolCall>{
                        id: toolCall.id ?: "",
                        'type: "function",
                        'function: {name: toolCall.name, arguments: toolCall.arguments}
                    });
                    completionMessages.push({role: ASSISTANT, tool_calls: chatCompletionToolCall});
                } else {
                ...
                completionMessages.push({role: "tool", content: message.content ?: "", tool_call_id: message.id ?: ""});
            }
        }
        return completionMessages;
    }

    private isolated function mapOpenAiToolCallsToToolCalls(chat:ChatCompletionMessageToolCalls openAiToolCalls)
        returns ToolCall[] {
        return from chat:ChatCompletionMessageToolCall toolCall in openAiToolCalls
            select {
    ...
    private isolated function mapChatCompletionFunctionsToChatCompletionTools(ChatCompletionFunctions[] tools)
        returns chat:ChatCompletionTool[] {
        return tools.'map(tool => <chat:ChatCompletionTool>{
            'type: "function",
            'function: {
                name: tool.name,
...
public isolated client class AzureOpenAiModel {
    *Model;
    ...
    isolated remote function chat(ChatMessage[] messages, ChatCompletionFunctions[] tools, string? stop = ()) returns ChatAssistantMessage|LlmError {
        ...
        azure_chat:ChatCompletionFunctionCall? toolCalls = message?.function_call;
        ChatAssistantMessage chatAssistantMessage = {role: ASSISTANT, content};
        if toolCalls is chat:ChatCompletionRequestAssistantMessage_function_call {
            chatAssistantMessage.toolCalls = [{name: toolCalls.name, arguments: toolCalls.arguments}];
        }
...
public isolated client class AnthropicModel {
    *Model;
    ...
    isolated remote function chat(ChatMessage[] messages, ChatCompletionFunctions[] tools = [], string? stop = ())
        ...
        return {role: ASSISTANT, toolCalls, content};
    }
}
...
public isolated client class MistralAiModel {
    *Model;
    ...
    isolated remote function chat(ChatMessage[] messages, ChatCompletionFunctions[] tools, string? stop = ())
    returns ChatAssistantMessage|LlmError {
        MistralMessages[] mistralMessages = self.mapMistralMessageRecords(messages);
            ...
            if message is ChatUserMessage {
                ...
                if message.toolCalls is () {
                    ...
                    mistral:FunctionCall functionCall = {
                        name: toolCalls is ToolCall[] ? toolCalls[0].name : "",
                        arguments: toolCalls is ToolCall[] ? toolCalls[0].arguments : ""
                    };
                    mistral:ToolCall[] toolCall = [
                        {
                            'function: functionCall,
                            id: toolCalls is ToolCall[] ? toolCalls[0].id ?: self.generateToolId() : self.generateToolId()
                        }
                    ];
                    mistral:AssistantMessage mistralAssistantMessage = {
                        role: ASSISTANT,
    ...
    private isolated function getAssistantMessage(mistral:ChatCompletionResponse response) returns ChatAssistantMessage|LlmError {
        ...
        ChatAssistantMessage chatAssistantMessage = {role: ASSISTANT};
        if content is string && content.length() > 0 {
            chatAssistantMessage.content = content;
        } else if content is mistral:TextChunk[] {
            chatAssistantMessage.content = string:'join("", ...content.'map(chunk => chunk.text));
        }

        if toolCalls is mistral:ToolCall[] {
            chatAssistantMessage.toolCalls = self.mapMistralToolCall(toolCalls);
        }

        if content is () && toolCalls is () {
            return error LlmError("Empty response from the model when using function call API", cause = content);
        }
        if content is mistral:ImageURLChunk[]|mistral:DocumentURLChunk[]|mistral:ReferenceChunk[] {
            return error LlmError("Unsupported content type", cause = content);
        }
        return chatAssistantMessage;
    }

    private isolated function mapMistralToolCall(mistral:ToolCall[] toolCalls) returns ToolCall[] {
        return from mistral:ToolCall toolCall in toolCalls
            select {
                name: toolCall.'function.name,
                id: toolCall.id,
