    ...
    *Model;
    ...
    isolated remote function chat(ChatMessage[] messages, ChatCompletionFunctions[] tools, string? stop = ()) returns ChatAssistantMessage|LlmError {
        ...
        if toolCalls is chat:ChatCompletionRequestAssistantMessage_function_call {
            return {role: ASSISTANT, toolCalls: [{name: toolCalls.name, arguments: toolCalls.arguments}]};
        }
        return {role: ASSISTANT, content: message?.content ?: ""};
    }
}
        ...
        foreach ChatMessage message in messages {
            if message is ChatUserMessage {
                ...
                if message.toolCalls is () {
                    ...
                    mistral:ToolCall[] mistaltToolCalls = toolCalls.map(toolCall => <mistral:ToolCall>{
                        'function: {
                            name: toolCall.name,
                            arguments: toolCall.arguments
                        },
                        id: toolCall.id ?: self.generateToolId()
                    });
                    mistral:AssistantMessage mistralAssistantMessage = {
                        role: ASSISTANT,
                        content: message.content,
                        toolCalls: mistaltToolCalls
                    };
                    mistralMessages.push(mistralAssistantMessage);
                }
